...
def post_save_account_receiver(sender, instance=None, created=False, *args, **kwargs):
    """
    Send email notification and generate credentials upon saving a new user instance.
    """
    if created:
        ...
        if instance.is_teacher:
            username, password = generate_teacher_credentials()
            instance.username = username
            instance.set_password(password)
            instance.save()
            # Send email with the generated credentials
            send_new_teacher_email.delay(instance.pk, password)
